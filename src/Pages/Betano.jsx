  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const filtered = data.filter(item => item.category === 'Betano');
        setDatas(data);
  ...
  return (
    <div className='py-10 bg-zinc-100'>
      <div className="2xl:max-w-[80%] xl:max-w-[90%] lg:max-w-[100%] max-w-[97%] mx-auto">
        {/* Header */}
        <div className="w-[100%] md:h-[220px] h-[200px] rounded-md flex justify-center items-end batano ">
          ...
          {filteredDatas.length === 0 ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-7">
              {currentItems.map((data) => (
                <div
                  key={data._id}
                  className="relative shadow-md flex flex-col justify-center items-center py-5 px-3 bg-white rounded-md"
                >
                  <p className="absolute top-2 left-2 text-gray-500 font-medium text-sm">Betslip</p>

                  <h2
                    className="font-medium py-10 text-3xl cursor-pointer break-words"
                  ...
                  {/* Copy icon */}
                  <div
                    className="absolute bottom-2 right-2 text-xl text-gray-500 cursor-pointer"
                    onClick={() => handleCopy(data.betslipCode, data._id)}
                    title={`Copy Betslip: ${data.betslipCode}`}
                  >
                    <FaRegCopy />
                  </div>
                  ...
                  {currentAdmin && (
                    <button className="bg-orange-600 rounded-bl-md p-1 absolute top-2 right-2 text-white hover:text-red-800" onClick={() => handleDelete(data._id)} title="Delete betslip">
                      <FaTrashAlt />
                    </button>
                  )}
                </div>
        ...
        {tooltip && (
          <div
            className={`fixed bottom-5 right-5 px-5 py-3 rounded-lg shadow-lg text-white font-semibold transition-opacity duration-300 ${
